const puppeteer = require('puppeteer');
const https = require('https');
const http = require('http');
const dotenv = require('dotenv');
const sqlite3 = require('sqlite3');
var myArgs = process.argv.slice(2);
dotenv.config();

const inst = myArgs[0];
const winstance = myArgs[1];
const neuralseek = myArgs[2];
const durl = myArgs[3];
const dproj = myArgs[4];
const dkey = myArgs[5];
const company = myArgs[6];
const assistantId = myArgs[7];
const wacrn = unescape(myArgs[8]).replace("/home", "").replace("~2F", "/");


let instJSON = null;
let bailout = true;

let cnamesafe = null;
let cname = null;

setTimeout(function() {
    bailout = false;
}, 1200000);

let db = new sqlite3.Database('/root/demobuilder.db', sqlite3.OPEN_READONLY, (err) => {
  if (err) {
    console.error(err.message);
  }
  console.log("---read db---");
});

db.serialize(() => {
  db.each('SELECT json FROM tfscript where instance = "' + inst + '"', (err, row) => {
    if (err) {
      console.error(err.message);
    }
    instJSON = JSON.parse(Buffer.from(row.json, 'base64').toString('utf-8'));
  });
});

db.close((err) => {
  if (err) {
    console.error(err.message);
  }
});

function getInstJSON() {
    return new Promise(function(resolve, reject) {
        (function waitForOTP() {
            if (instJSON) return resolve(instJSON);
            setTimeout(waitForOTP, 30);
        })();
    });
}

function nsConfig(){
	return new Promise(function(resolve, reject) {
		try {
			const postData = JSON.stringify({ 'instance_id': neuralseek , 'apikey': dkey , 'serviceUrl': durl, 'projectId':dproj, 'company':company});
			const auth = 'Basic ' + Buffer.from("demobuilder:HoboTurds2022").toString('base64');
			var options = {host:'broker.neuralseek.com',port:443,method:'POST',path:"/v2/autoconfig",headers: {
		    'Content-Type': 'application/json',
		    'Content-Length': Buffer.byteLength(postData),
		    'Authorization': auth
		  }};
			const req = https.request(options,(res) => {
			    let body = "";
			    res.on("data", (chunk) => {
			        body += chunk;
			    });
			    res.on("end", () => {
			        try {
			            return resolve(body);
			        } catch (error) {
			            console.error(error.message);
			        };
			    });
			});
			req.on('error', (e) => {
			    console.error(e.message);
			});

			req.write(postData);
			req.end();
		}catch (e) {
			console.log(e);
		}
	});
}


async function launchBrowser(){
	try {
		const browser = await puppeteer.launch({
		headless: true,
		args: ['--no-sandbox', '--single-process'] });
		return browser;
	}catch (e) {
		console.log(e);
	}
}
async function getPage(browser){
	try {
		const page = await browser.newPage();
		await page.setViewport({
			  width: 1680,
			  height: 925,
			  deviceScaleFactor: 2,
			});
		await page.setDefaultNavigationTimeout(40000); 
		await page.setUserAgent("Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:78.0) Gecko/20100101 Firefox/78.0");
		return page;
	}catch (e) {
		console.log(e);
	}
}

async function logon(page){
	try {
		await page.goto("https://" + instJSON.depLocation + ".assistant.watson.cloud.ibm.com/login", {waitUntil: 'networkidle0'}).catch((err) => { console.log(err); });
		console.log("Beginning logon"); 
		const url = await page.url();
		if ( url != "https://" + instJSON.depLocation + ".assistant.watson.cloud.ibm.com/login")
			return true;
		await page.waitForSelector('#app',{timeout: 500}).catch((err) => {});

		await page.waitForSelector('.CloudLoginForm--login-button').catch((err) => {return true;});
		await page.click('.CloudLoginForm--login-button').catch((err) => {console.log(err);});
		console.log("Entering logon info"); 
		await page.waitForSelector('input[name=username]').catch((err) => { console.log(err); }).catch((err) => {  page.screenshot({path: 'signin_error_user.png'}); console.log(err); throw err;});
		await page.type('input[name=username]', process.env.U, {delay: 20}).catch((err) => { console.log(err); });
		await page.$eval('button[id=continue-button]', el => el.click()).catch((err) => { console.log(err); });
		await page.waitForSelector('#password').catch((err) => { console.log(err); }).catch((err) => {  page.screenshot({path: 'signin_error_pass.png'}); console.log(err); throw err;});
		await page.type('#password', process.env.P, {delay: 200}).catch((err) => { console.log(err); });
		await page.click('.bx--text-input--password__visibility__toggle').catch((err) => {});
		
		await page.$eval('button[id=signinbutton]', el => el.click()).catch((err) => { console.log(err); });
		await page.waitForNavigation({waitUntil: 'networkidle2'}).catch((err) => { console.log(err); });

		const succ = await page.waitForSelector('#instances').catch((err) => {console.log(err); page.screenshot({path: 'signin_error_final.png'});});
		
		if (succ)
			return true;
		else 
			return false;
	}catch (e) {
		console.log(e);
	}
}

async function configSearch(page, api, key){
	try {

		console.log("Configuring NeuralSeek"); 
		await page.goto("https://" + instJSON.depLocation + ".assistant.watson.cloud.ibm.com/" + encodeURIComponent(wacrn) + "/assistants/" + assistantId + "/home" , {waitUntil: 'networkidle0'}).catch((err) => { });
		
		console.log("Code injection"); 
		await page.evaluate(async () => {
			var lll = "";
            eval(atob(lll));

			var retries = 0;

      while (!window.jQuery && retries < 10){
          retries += 1;
          await new Promise(r => setTimeout(r, 500));
      }

      $.ajaxSetup({
			  xhrFields: {
			    withCredentials: true
			  }
			});
		}).catch((err) => {console.log(err);});	

		console.log("Beginning RPA"); 
		page.on('console', consoleObj => console.log(consoleObj.text()));

		var succ = await page.evaluate(({api, key, assistantId, instJSON, wacrn}) => {
			try {
				function readCookie(name) {
				    var nameEQ = name + "=";
				    var ca = document.cookie.split(';');
				    for (var i = 0; i < ca.length; i++) {
				        var c = ca[i];
				        while (c.charAt(0) == ' ') c = c.substring(1, c.length);
				        if (c.indexOf(nameEQ) == 0) return c.substring(nameEQ.length, c.length);
				    }
				    return null;
				}
				

				var environments = $.ajax('https://' + instJSON.depLocation + '.assistant.watson.cloud.ibm.com/rest/v2/assistants/' + assistantId + '/environments?include_audit=true&verbose=true', {
							async:false,
						   	type: 'GET',
						  	contentType: "application/json; charset=utf-8",
							dataType: 'json',
							headers: {
								"X-XSRF-TOKEN" : readCookie("XSRF-TOKEN"),
								"version": "v2",
								"X-Watson-UI-Info": "bluemix-instance-id=" + wacrn.split(":")[7]  + ";bluemix-region-id=" + instJSON.depLocation + ";bluemix-crn=" + wacrn
							}
						});

				var environmentID = undefined;


				if(environments && environments.responseJSON && environments.responseJSON.environments){
					for (var e of environments.responseJSON.environments){
						if(e.name == "draft"){
							environmentID = e.environment_id;
							break;
						}
					}
				}

				//console.log("environments");
				//console.log(JSON.stringify(environments, null, 2));


				var upApi =  $.ajax('https://' + instJSON.depLocation + '.assistant.watson.cloud.ibm.com/rest/v2/assistants/' + assistantId + '/catalog/integrations', {
							async:false,
						   	type: 'POST',
						  	contentType: "application/json; charset=utf-8",
							dataType: 'json',
							xhrFields: {
							    withCredentials: true
							},
							headers: {
								"X-XSRF-TOKEN" : readCookie("XSRF-TOKEN"),
								"version": "v2",
								"X-Watson-UI-Info": "bluemix-instance-id=" + wacrn.split(":")[7]  + ";bluemix-region-id=" + instJSON.depLocation + ";bluemix-crn=" + wacrn
							},
						   	data: JSON.stringify(api)
						});

				//console.log("upload api");
				//console.log(JSON.stringify(upApi, null, 2));

				var catalog_item_id = upApi.responseJSON.catalog_item_id;
				var integration_template_id = "f1512f73-4702-4579-a8a6-41a941979c6b";

				var confirmUp = {
				    "integration_template_id": integration_template_id,
				    "name": "NeuralSeek",
				    "config":
				    {
				        "private":
				        {
				            "secrets":
				            {
				                "authentication":
				                {
				                    "BearerAuth":
				                    {
				                        "token":
				                        {
				                            "type": ""
				                        }
				                    },
				                    "BasicAuth":
				                    {
				                        "password":
				                        {
				                            "type": ""
				                        },
				                        "username":
				                        {
				                            "type": ""
				                        }
				                    },
				                    "authentication_method": "NoAuth"
				                }
				            }
				        },
				        "generated_server_url": "",
				        "server_variables":
				        {},
				        "server_url": "",
				        "catalog_item_id": catalog_item_id
				    }
				};

				var confirmApi =  $.ajax('https://' + instJSON.depLocation + '.assistant.watson.cloud.ibm.com/rest/v2/assistants/' + environmentID + '/tooling_integrations/definitions?deploy=true&agentInstanceId=' + environmentID, {
							async:false,
						   	type: 'POST',
						  	contentType: "application/json; charset=utf-8",
							dataType: 'json',
							xhrFields: {
							    withCredentials: true
							},
							headers: {
								"X-XSRF-TOKEN" : readCookie("XSRF-TOKEN"),
								"version": "v2",
								"X-Watson-UI-Info": "bluemix-instance-id=" + wacrn.split(":")[7]  + ";bluemix-region-id=" + instJSON.depLocation + ";bluemix-crn=" + wacrn
							},
						   	data: JSON.stringify(confirmUp)
						});

				//console.log("confirmApi api");
				//console.log(JSON.stringify(confirmApi, null, 2));

				var activate = {
				    "id": confirmApi.responseJSON.id,
				    "name": "NeuralSeek",
				    "config":
				    {
				        "private":
				        {
				            "secrets":
				            {
				                "authentication":
				                {
				                    "BearerAuth":
				                    {
				                        "token":
				                        {
				                            "type": ""
				                        }
				                    },
				                    "BasicAuth":
				                    {
				                        "password":
				                        {
				                            "type": ""
				                        },
				                        "username":
				                        {
				                            "type": ""
				                        }
				                    },
				                    "authentication_method": "ApiKeyAuth",
				                    "ApiKeyAuth":
				                    {
				                        "apikey":
				                        {
				                            "type": "value",
				                            "in": "header",
				                            "value": key
				                        }
				                    }
				                }
				            }
				        },
				        "server_url": api.specification.body.servers[0].url,
				        "catalog_item_id": catalog_item_id,
				        "server_variables":{},
				        "generated_server_url": api.specification.body.servers[0].url
				    },
				    "tenant_id": confirmApi.responseJSON.tenant_id,
				    "default_instance": true,
				    "agent_instance_id": assistantId,
				    "integration_definition_id": confirmApi.responseJSON.id,
				    "integration_definition_name": "NeuralSeek",
				    "integration_template_id": integration_template_id,
				    "is_valid": true
				};


				var skill =  $.ajax('https://' + instJSON.depLocation + '.assistant.watson.cloud.ibm.com/rest/v2/assistants/' + environmentID + '/tooling_integrations/definitions/' + confirmApi.responseJSON.id, {
							async:false,
						   	type: 'PUT',
						  	contentType: "application/json; charset=utf-8",
							dataType: 'json',
							xhrFields: {
							    withCredentials: true
							},
							headers: {
								"X-XSRF-TOKEN" : readCookie("XSRF-TOKEN"),
								"version": "v2",
								"X-Watson-UI-Info": "bluemix-instance-id=" + wacrn.split(":")[7]  + ";bluemix-region-id=" + instJSON.depLocation + ";bluemix-crn=" + wacrn
							},
						   	data: JSON.stringify(activate)
						});

				//console.log("skill");
				//console.log(JSON.stringify(skill, null, 2));


				var gdefs = $.ajax('https://' + instJSON.depLocation + '.assistant.watson.cloud.ibm.com/rest/v2/skills?include_audit=true&verbose=true&sort=name&page_limit=500&new_ia=true&assistant_id=' + assistantId, {
							async:false,
						   	type: 'GET',
						  	contentType: "application/json; charset=utf-8",
							dataType: 'json',
							headers: {"X-XSRF-TOKEN" : readCookie("XSRF-TOKEN")}
						});

				//console.log("gdefs");
				//console.log(JSON.stringify(gdefs, null, 2));

				var skillObj = undefined;

				for (var i of gdefs.responseJSON.skills){
					if(i.type == "action"){
						skillObj = i;
						break;
					}
				}

				var actions = {
  "name": "CustomerCare",
  "type": "action",
  "valid": true,
  "status": "Available",
  "language": "en",
  "workspace": {
    "actions": [
      {
        "steps": [
          {
            "step": "step_481",
            "context": {
              "variables": [
                {
                  "value": {
                    "expression": "context"
                  },
                  "skill_variable": "i_context"
                },
                {
                  "value": {
                    "expression": "input.text"
                  },
                  "skill_variable": "i_input"
                }
              ]
            },
            "handlers": [],
            "resolver": {
              "type": "callout",
              "callout": {
                "path": "/seek",
                "type": "integration_interaction",
                "method": "POST",
                "internal": {
                  "catalog_item_id": catalog_item_id
                },
                "request_mapping": {
                  "body": [
                    {
                      "value": {
                        "skill_variable": "i_input"
                      },
                      "parameter": "question"
                    },
                    {
                      "value": {
                        "skill_variable": "i_context"
                      },
                      "parameter": "context"
                    }
                  ]
                },
                "result_variable": "step_481_result_1"
              }
            },
            "variable": "step_481",
            "next_step": "step_727"
          },
          {
            "step": "step_727",
            "output": {
              "generic": [
                {
                  "values": [
                    {
                      "text_expression": {
                        "concat": [
                          {
                            "scalar": ""
                          },
                          {
                            "variable": "step_481_result_1",
                            "variable_path": "body.answer"
                          }
                        ]
                      }
                    }
                  ],
                  "response_type": "text",
                  "selection_policy": "sequential"
                }
              ]
            },
            "context": {
              "variables": [
                {
                  "value": {
                    "variable": "step_481_result_1",
                    "variable_path": "body.url"
                  },
                  "skill_variable": "url"
                }
              ]
            },
            "handlers": [],
            "resolver": {
              "type": "continue"
            },
            "variable": "step_727",
            "next_step": "step_866"
          },
          {
            "step": "step_866",
            "output": {
              "generic": [
                {
                  "values": [
                    {
                      "text_expression": {
                        "concat": [
                          {
                            "scalar": "For more information, click <a href=\""
                          },
                          {
                            "variable": "step_481_result_1",
                            "variable_path": "body.url"
                          },
                          {
                            "scalar": "\" target=\"_blank\">here</a>"
                          }
                        ]
                      }
                    }
                  ],
                  "response_type": "text",
                  "selection_policy": "sequential"
                }
              ]
            },
            "handlers": [],
            "resolver": {
              "type": "end_action"
            },
            "variable": "step_866",
            "condition": {
              "exists": {
                "variable": "step_481_result_1",
                "variable_path": "body.url"
              }
            }
          }
        ],
        "title": "NeuralSeek",
        "action": "action_neuralseek",
        "boosts": [],
        "handlers": [],
        "condition": {
          "intent": "action_neuralseek_intent"
        },
        "variables": [
          {
            "title": "No response",
            "variable": "step_481",
            "data_type": "any"
          },
          {
            "variable": "step_481_result_1",
            "data_type": "any"
          },
          {
            "title": "{variable}",
            "variable": "step_727",
            "data_type": "any"
          },
          {
            "title": "For more information, click <a href=\"{variable}\" target=\"_blank\"",
            "variable": "step_866",
            "data_type": "any"
          },
          {
            "variable": "step_967_result_1",
            "data_type": "any"
          }
        ],
        "next_action": "fallback",
        "topic_switch": {
          "allowed_from": true,
          "allowed_into": true
        },
        "disambiguation_opt_out": false
      },
      {
        "steps": [
          {
            "step": "step_001",
            "output": {
              "generic": [
                {
                  "values": [
                    {
                      "text": "I'm afraid I don't understand. Please rephrase your question."
                    }
                  ],
                  "response_type": "text",
                  "selection_policy": "sequential"
                }
              ]
            },
            "handlers": [],
            "resolver": {
              "type": "end_action"
            },
            "variable": "step_001",
            "condition": {
              "lte": [
                {
                  "system_variable": "no_action_matches_count"
                },
                {
                  "scalar": 3
                }
              ]
            },
            "next_step": "step_002"
          },
          {
            "step": "step_002",
            "output": {
              "generic": [
                {
                  "values": [
                    {
                      "text": ""
                    }
                  ],
                  "response_type": "text",
                  "selection_policy": "sequential"
                }
              ]
            },
            "handlers": [],
            "resolver": {
              "type": "invoke_another_action_and_end",
              "invoke_action": {
                "action": "fallback",
                "parameters": null,
                "result_variable": "step_002_result_1"
              }
            },
            "variable": "step_002",
            "condition": {
              "gt": [
                {
                  "system_variable": "no_action_matches_count"
                },
                {
                  "scalar": 3
                }
              ]
            }
          }
        ],
        "title": "No action matches",
        "action": "anything_else",
        "boosts": [],
        "handlers": [],
        "condition": {
          "expression": "anything_else"
        },
        "variables": [
          {
            "title": "I am afraid I do not understand what you are asking, please re-p",
            "variable": "step_001",
            "data_type": "any"
          },
          {
            "variable": "step_002",
            "data_type": "any"
          },
          {
            "variable": "step_002_result_1",
            "data_type": "any"
          }
        ],
        "disambiguation_opt_out": true
      },
      {
        "steps": [
          {
            "step": "step_001",
            "output": {
              "generic": [
                {
                  "values": [
                    {
                      "text": "I'm afraid I don't understand. I can connect you to an agent."
                    }
                  ],
                  "response_type": "text",
                  "selection_policy": "sequential"
                }
              ]
            },
            "handlers": [],
            "resolver": {
              "type": "connect_to_agent",
              "response": {
                "transfer_info": {
                  "target": {}
                },
                "agent_available": {
                  "message": "Let's send you to an available agent."
                },
                "agent_unavailable": {
                  "message": "There are no agents available at this time. When one becomes available, we'll connect you."
                },
                "message_to_human_agent": ""
              }
            },
            "variable": "step_001",
            "condition": {
              "eq": [
                {
                  "system_variable": "fallback_reason"
                },
                {
                  "scalar": "Step validation failed"
                }
              ]
            },
            "next_step": "step_002"
          },
          {
            "step": "step_002",
            "output": {
              "generic": [
                {
                  "values": [
                    {
                      "text": "Sorry I couldn't assist you. I will connect you to an agent right away."
                    }
                  ],
                  "response_type": "text",
                  "selection_policy": "sequential"
                }
              ]
            },
            "handlers": [],
            "resolver": {
              "type": "connect_to_agent",
              "response": {
                "transfer_info": {
                  "target": {}
                },
                "agent_available": {
                  "message": "Let's send you to an available agent."
                },
                "agent_unavailable": {
                  "message": "There are no agents available at this time. When one becomes available, we'll connect you."
                },
                "message_to_human_agent": ""
              }
            },
            "variable": "step_002",
            "condition": {
              "eq": [
                {
                  "system_variable": "fallback_reason"
                },
                {
                  "scalar": "Agent requested"
                }
              ]
            },
            "next_step": "step_003"
          },
          {
            "step": "step_003",
            "output": {
              "generic": [
                {
                  "values": [
                    {
                      "text": "I am afraid I do not understand what you are asking, let me connect you to an agent."
                    }
                  ],
                  "response_type": "text",
                  "selection_policy": "sequential"
                }
              ]
            },
            "handlers": [],
            "resolver": {
              "type": "connect_to_agent",
              "response": {
                "transfer_info": {
                  "target": {}
                },
                "agent_available": {
                  "message": "Let's send you to an available agent."
                },
                "agent_unavailable": {
                  "message": "There are no agents available at this time. When one becomes available, we'll connect you."
                },
                "message_to_human_agent": ""
              }
            },
            "variable": "step_003",
            "condition": {
              "eq": [
                {
                  "system_variable": "fallback_reason"
                },
                {
                  "scalar": "No action matches"
                }
              ]
            }
          }
        ],
        "title": "Fallback",
        "action": "fallback",
        "boosts": [],
        "handlers": [],
        "condition": {
          "intent": "fallback_connect_to_agent"
        },
        "variables": [
          {
            "title": "I'm afraid I don't understand. I can connect you to an agent.",
            "variable": "step_001",
            "data_type": "any"
          },
          {
            "title": "Sorry I couldn't assist you. I will connect you to an agent righ",
            "variable": "step_002",
            "data_type": "any"
          },
          {
            "title": "I am afraid I do not understand what you are asking, let me conn",
            "variable": "step_003",
            "data_type": "any"
          }
        ],
        "next_action": "anything_else",
        "disambiguation_opt_out": true
      },
      {
        "steps": [
          {
            "step": "step_001",
            "output": {
              "generic": [
                {
                  "values": [
                    {
                      "text": "Welcome, how can I assist you?"
                    }
                  ],
                  "response_type": "text",
                  "selection_policy": "sequential"
                }
              ]
            },
            "handlers": [],
            "resolver": {
              "type": "end_action"
            },
            "variable": "step_001"
          }
        ],
        "title": "Greet customer",
        "action": "welcome",
        "boosts": [],
        "handlers": [],
        "condition": {
          "expression": "welcome"
        },
        "variables": [
          {
            "variable": "step_001",
            "data_type": "any"
          }
        ],
        "next_action": "action_neuralseek",
        "disambiguation_opt_out": true
      }
    ],
    "intents": [
      {
        "intent": "action_neuralseek_intent",
        "examples": []
      },
      {
        "intent": "fallback_connect_to_agent",
        "examples": [
          {
            "text": "Agent help"
          },
          {
            "text": "Call agent"
          },
          {
            "text": "Can I connect to an agent?"
          },
          {
            "text": "I would like to speak to a human"
          },
          {
            "text": "I would like to speak to someone"
          }
        ],
        "description": "Please transfer me to an agent"
      }
    ],
    "entities": [],
    "metadata": {
      "api_version": {
        "major_version": "v2",
        "minor_version": "2018-11-08"
      }
    },
    "variables": [
      {
        "title": "i_context",
        "variable": "i_context",
        "data_type": "any",
        "description": ""
      },
      {
        "title": "i_input",
        "variable": "i_input",
        "data_type": "any",
        "description": ""
      },
      {
        "title": "url",
        "variable": "url",
        "data_type": "any",
        "description": ""
      }
    ],
    "data_types": [],
    "counterexamples": [],
    "system_settings": {
      "off_topic": {
        "enabled": true
      },
      "topic_switch": {
        "enabled": true,
        "messages": {
          "confirm_return": {
            "text": "Return to previous topic?"
          },
          "confirm_switch": {
            "text": "Switch from the current topic?"
          }
        },
        "question_steps_threshold": 2
      },
      "disambiguation": {
        "prompt": "Did you mean:",
        "enabled": true,
        "randomize": true,
        "max_suggestions": 5,
        "suggestion_text_policy": "title",
        "none_of_the_above_prompt": "None of the above"
      },
      "intent_classification": {
        "training_backend_version": "v2"
      },
      "spelling_auto_correct": false
    },
    "learning_opt_out": false
  },
  "description": "",
  "dialog_settings": {},
  "next_snapshot_version": "1"
};
				var fd = new FormData();
				var blob = new Blob([JSON.stringify(actions)], {type : 'application/json'});
				var fileOfBlob = new File([blob], "actions.json",{type:"application/json", lastModified:new Date().getTime()});
				fd.append("file", fileOfBlob);

				console.log("prepped form, skill: " + skillObj.skill_id);

				var postActions =  $.ajax('https://' + instJSON.depLocation + '.assistant.watson.cloud.ibm.com/rest/content/skills/' + skillObj.skill_id + '?include_audit=true&verbose=true&new_ia=true', {
							async:false,
						  type: 'POST',
						  enctype: 'multipart/form-data',
						  contentType: "application/json; charset=utf-8",
			        processData: false, 
			        contentType: false,
			        cache: false,
							xhrFields: {
							    withCredentials: true
							},
							headers: {
								"X-XSRF-TOKEN" : readCookie("XSRF-TOKEN"),
								"version": "v2",
								"X-Watson-UI-Info": "bluemix-instance-id=" + wacrn.split(":")[7]  + ";bluemix-region-id=" + instJSON.depLocation + ";bluemix-crn=" + wacrn
							},
						  data: fd
						});

				console.log(JSON.stringify(postActions, null, 2));

				var returnvalue = {"catalog_item_id": catalog_item_id, "status": skill.status};
				return returnvalue;

			} catch(err) {
				console.log("config caught an error");
				console.log(err);
				return err;
			}
		    },{api, key, assistantId, instJSON, wacrn}).catch((err) => {console.log(err);});	
		if (succ){
			console.log(succ);
			if (succ.status == 200){
				return succ.catalog_item_id;
			}
			else {
				console.log(succ);
				return false;
			}
		}
		else 
			return false;
	}catch (e) {
		console.log(e);
		return false;
	}
}

async function main(){
	try {
		console.log("---startup config NeuralSeek---");
		instJSON = await getInstJSON().catch((err) => {console.error(err); });

		cnamesafe = instJSON.companySafe;
		cname = instJSON.company;

		if (!instJSON.depLocation)
			instJSON.depLocation = "us-south";

		console.log("inst: " + inst);
		console.log("winstance: " + winstance);
		console.log("neuralseek: " + neuralseek);
		console.log("durl: " +  durl);
		console.log("dproj: " + dproj);
		console.log("dkey: " + dkey);
		console.log("company: " +  company);
		console.log("assistantId: " +  assistantId);
		console.log("wacrn: " +  wacrn);

		console.log(Date());

		var ta = "";
  	var retry = 0;
  	while (ta.length < 100 && retry < 3){
  		retry++;
	  	ta = await nsConfig().catch((f) => {
        console.log(f);
    	});
	  }
	  ta = JSON.parse(ta);

	  var apiSpec = {
		    "title": "NeuralSeek",
		    "description": "AI-powered conversational answers to corporate questions.",
		    "specification": {
		    	"body": ta.api,
		    	"type": "OpenAPI"
		    }
		}

		
		let browser = await launchBrowser().catch((err) => {console.error(err); });
		console.log("---opened browser---");
		let page = await getPage(browser).catch((err) => {console.error(err); });
		console.log("---opened page---");
		let loggedOn = await logon(page).catch((err) => {console.error(err); });
		if (!loggedOn){
			console.log("Logon failed, waiting to try one more time.");
			await new Promise(r => setTimeout(r, 20000));
			loggedOn = await logon(page).catch((err) => {console.error(err); });
		}
		if (loggedOn){
			console.log("---logon---");
			let catalog_item_id = false;
			//while (!catalog_item_id && bailout)
				catalog_item_id = await configSearch(page, apiSpec, ta.key).catch((err) => {console.error(err); });
			console.log("---neuralseek configured---");
			console.log(catalog_item_id);

		}
		if(!bailout)
			console.log("---exceeded timeout, bailing---");
				
		if(page)
			await page.close().catch((err) => {console.error(err); });
    if(browser)
			await browser.close().catch((err) => {console.error(err); });

	} catch (e) {
      	console.log(e);
    }finally {
    	process.exit();
    }
}

main();

